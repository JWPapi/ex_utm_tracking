...
<div class="tw-grid lg:tw-grid-cols-4 tw-gap-4 tw-bg-white tw-border-solid tw-border tw-border-gray-400 tw-p-5 tw-mt-4">
    ...
    <div>
      ...
      <div class="tw-flex tw-flex-col tw-mt-4">
        <div class=""><label class="tw-font-bold tw-text-gray-600" for="input-s_conversion"><?php esc_html_e('Conversions', AFL_WC_UTM_TEXTDOMAIN); ?></label></div>
        <div class="tw-mt-2">
          <select class="tw-w-full" id="input-s_conversion" name="s_conversion">
            <option value="">-- Please select --</option>
            <option value="any" <?php selected($form_values['s_conversion'], 'any', true); ?>>Any conversion</option>
            <option value="no" <?php selected($form_values['s_conversion'], 'no', true); ?>>No conversion</option>
            <option value="lead_and_order" <?php selected($form_values['s_conversion'], 'lead_and_order', true); ?>>Has both lead & order</option>
            <option value="lead" <?php selected($form_values['s_conversion'], 'lead', true); ?>>Has lead</option>
            <option value="lead_only" <?php selected($form_values['s_conversion'], 'lead_only', true); ?>>Has lead only</option>
            <option value="order" <?php selected($form_values['s_conversion'], 'order', true); ?>>Has order</option>
            <option value="order_only" <?php selected($form_values['s_conversion'], 'order_only', true); ?>>Has order only</option>
          </select>
        </div>
      </div>
    ...
    <div>
      <div class="tw-text-lg tw-font-bold"><?php esc_html_e('Date Updated', AFL_WC_UTM_TEXTDOMAIN); ?></div>

      <div class="tw-flex tw-flex-col tw-mt-4">
        <div class=""><label class="tw-font-bold tw-text-gray-600" for="input-s_date_last_updated_from">From Date</label></div>
        <div class="tw-mt-2"><input type="date" class="tw-w-full" id="input-s_date_last_updated_from"  name="s_date_updated[from]" value="<?php echo esc_attr($form_values['s_date_updated']['from']); ?>"></div>
      </div>

      <div class="tw-flex tw-flex-col tw-mt-4">
        <div class=""><label class="tw-font-bold tw-text-gray-600" for="input-s_date_updated_to">To Date</label></div>
        <div class="tw-mt-2"><input type="date" class="tw-w-full" id="input-s_date_updated_to"  name="s_date_updated[to]" value="<?php echo esc_attr($form_values['s_date_updated']['to']); ?>"></div>
      </div>
      ...
      <div class="tw-flex tw-flex-col tw-mt-4">
        ...
        <div class="tw-mt-2"><input type="date" class="tw-w-full" id="input-s_date_first_visit_from"  name="s_sess_visit[from]" value="<?php echo esc_attr($form_values['s_sess_visit']['from']); ?>"></div>
      </div>
